...
func Merge(ctx context.Context, layers []Layer, dest io.Writer, opt MergeOption) ([]digest.Digest, error) {
	...
	if opt.FsVersion == "6" {
		...
		for idx := range layers {
			...
			_, err = os.Stat(blobMetaHeaderPath)
			if err != nil {
			...
			assembledBuffer := make([]byte, totalSize)

			copy(assembledBuffer[0:uncompressedSize], metaContent)
			copy(assembledBuffer[alignedUncompressedSize:alignedUncompressedSize+len(headerContent)], headerContent)
			assembledFileName := fmt.Sprintf("%s.blob.meta", digestHex)
			assembledFilePath := filepath.Join(workDir, assembledFileName)
			if err := os.WriteFile(assembledFilePath, assembledBuffer, 0644); err != nil {
				return nil, errors.Wrap(err, "write assembled meta file")
			}

			assembledRa, err := local.OpenReader(assembledFilePath)
			if err != nil {
				return nil, errors.Wrap(err, "open assembled meta file")
			}
